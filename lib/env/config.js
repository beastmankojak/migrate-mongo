...
async function loadCommonjsModule(configPath) {
  return await Promise.resolve(moduleLoader.require(configPath));
}

async function loadEsmModule(configPath) {
...
module.exports = {
  ...
  async read() {
    ...
    const configPath = getConfigPath();
    try {
      const module = await loadCommonjsModule(configPath);
      if (module.default?.moduleSystem === 'esm') {
        return await loadEsmModule(configPath);
      } else {
        return module;
      }
    } catch (e) {
